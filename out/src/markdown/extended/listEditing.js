...
function activate(context) {
    context.subscriptions.push(
        vscode.commands.registerCommand('markdown.extension.onEnterKey', onEnterKey), 
        vscode.commands.registerCommand('markdown.extension.onCtrlEnterKey', () => { onEnterKey('ctrl'); }), 
        vscode.commands.registerCommand('markdown.extension.onShiftEnterKey', () => { onEnterKey('shift'); }), 
        vscode.commands.registerCommand('markdown.extension.onTabKey', onTabKey), 
        vscode.commands.registerCommand('markdown.extension.onShiftTabKey', () => { onTabKey('shift'); }), 
        vscode.commands.registerCommand('markdown.extension.onBackspaceKey', onBackspaceKey), 
        vscode.commands.registerCommand('markdown.extension.checkTaskList', checkTaskList), 
        vscode.commands.registerCommand('markdown.extension.onMoveLineDown', onMoveLineDown), 
        vscode.commands.registerCommand('markdown.extension.onMoveLineUp', onMoveLineUp), 
        vscode.commands.registerCommand('markdown.extension.onCopyLineDown', onCopyLineDown), 
        vscode.commands.registerCommand('markdown.extension.onCopyLineUp', onCopyLineUp), 
        vscode.commands.registerCommand('markdown.extension.onIndentLines', onIndentLines), 
        vscode.commands.registerCommand('markdown.extension.onOutdentLines', onOutdentLines)
    );
}
...
function onTabKey(modifiers) {
    ...
    if (!editor.selection.isEmpty || modifiers === 'shift' || (match = /^\s*([-+*]|[0-9]+[.)]) +(\[[ x]\] +)?/.exec(lineText)) !== null && cursorPos.character <= match[0].length) {
        ...
        return vscode.commands.executeCommand(command).then(() => fixMarker());
    }
    else {
        return asNormal('tab', modifiers);
    }
...
function onBackspaceKey() {
    ...
    if (!editor.selection.isEmpty) {
        ...
        return vscode.commands.executeCommand('editor.action.outdentLines').then(() => fixMarker());
    }
    else if (/^([-+*]|[0-9]+[.)]) $/.test(textBeforeCursor)) {
        // e.g. textBeforeCursor == '- ', '1. '
        return editor.edit(editBuilder => {
            editBuilder.replace(new vscode.Range(cursor.with({ character: 0 }), cursor), ' '.repeat(textBeforeCursor.length));
        }).then(() => fixMarker(findNextMarkerLineNumber()));
    }
    else if (/^([-+*]|[0-9]+[.)]) (\[[ x]\] )$/.test(textBeforeCursor)) {
        // e.g. textBeforeCursor == '- [ ]', '1. [x]'
        return deleteRange(editor, new vscode.Range(cursor.with({ character: textBeforeCursor.length - 4 }), cursor)).then(() => fixMarker(findNextMarkerLineNumber()));
    }
    else {
        return asNormal('backspace');
    }
...
function asNormal(key, modifiers) {
    switch (key) {
        ...
        case 'tab':
            if (vscode.workspace.getConfiguration('emmet').get('triggerExpansionOnTab')) {
...
function fixMarker(line) {
    if (!vscode.workspace.getConfiguration('markdown.extension.orderedList').get('autoRenumber'))
        return;
    if (vscode.workspace.getConfiguration('markdown.extension.orderedList').get('marker') == 'one')
        return;
    let editor = vscode.window.activeTextEditor;
    if (line === undefined) {
    ...
    if ((matches = /^(\s*)([0-9]+)([.)])( +)/.exec(currentLineText)) !== null) { // ordered list
        ...
        return editor.edit(editBuilder => {
            ...
            editBuilder.replace(new vscode.Range(line, leadingSpace.length, line, leadingSpace.length + listIndent), fixedMarkerString);
...
function onMoveLineUp() {
    return vscode.commands.executeCommand('editor.action.moveLinesUpAction')
        .then(() => fixMarker());
}
function onMoveLineDown() {
    return vscode.commands.executeCommand('editor.action.moveLinesDownAction')
        .then(() => fixMarker(findNextMarkerLineNumber(vscode.window.activeTextEditor.selection.start.line - 1)));
}
function onCopyLineUp() {
    return vscode.commands.executeCommand('editor.action.copyLinesUpAction')
        .then(() => fixMarker());
}
