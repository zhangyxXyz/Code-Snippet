...
let decorTypes = {
    "baseColor": vscode.window.createTextEditorDecorationType({
        "dark": { "color": "#EEFFFF" },
        "light": { "color": "000000" }
    }),
    "gray": vscode.window.createTextEditorDecorationType({
        "rangeBehavior": 1,
        "dark": { "color": "#636363" },
        "light": { "color": "#CCC" }
    }),
    "lightBlue": vscode.window.createTextEditorDecorationType({
        "color": "#4080D0"
    }),
    ...
    "codeSpan": vscode.window.createTextEditorDecorationType({
        "rangeBehavior": 1,
...
function updateDecorations(editor) {
    if (!vscode.workspace.getConfiguration('markdown.extension.syntax').get('decorations'))
        return;
    if (editor === undefined) {
        editor = vscode.window.activeTextEditor;
    }
    if (!util.isMdEditor(editor)) {
        return;
    }
    // Clean decorations
    for (const decorTypeName in decorTypes) {
    ...
    editor.document.getText().split(/\r?\n/g).forEach((lineText, lineNum) => {
        let appliedMappings = vscode.workspace.getConfiguration('markdown.extension.syntax').get('plainTheme') ? Object.assign({}, regexDecorTypeMapping, regexDecorTypeMappingPlainTheme) :
            ...
            if (appliedMappings.hasOwnProperty(reText)) {
                ...
                while ((match = regex.exec(lineText)) !== null) {
                    ...
                    for (let i = 0; i < decorTypeNames.length; i++) {
                        let range = new vscode.Range(lineNum, startIndex, lineNum, startIndex + match[i + 1].length);
